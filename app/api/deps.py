...
async def provide_task_repository(db_session: AsyncSession = Depends(provide_db_session)) -> TaskRepository:
    """Провайдер репозитория задач"""
    return TaskRepository(db_session)

async def provide_task_service(task_repository: TaskRepository = Depends(provide_task_repository)) -> TaskService:
