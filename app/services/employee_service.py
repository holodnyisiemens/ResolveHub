...
class EmployeeService:
    ...
    async def update(
        self, employee_id: int, employee_data: EmployeeUpdateDTO
    ) -> EmployeeDTO:
        employee = await self.employee_repo.get_by_id(employee_id)
        if not employee:
            raise HTTPException(
        ...
        if employee_data.email is not None:
            existing_employee = await self.employee_repo.get_by_email(
                employee_data.email
            )
        ...
        if employee_data.username is not None:
            existing_employee = await self.employee_repo.get_by_username(
